...
Page({
    ...
    fetchRate:function(){
        ...
        if(self.data.date && self.data.debitCurrency && self.data.tradeCurrency){
            wx.request({
              url: app.globalData.gwapi + '/v1/wx/huobi/' + self.data.debitCurrency.description + '/' + self.data.tradeCurrency.description + '/' + self.data.date,
                header:{
                    "contentType":"application/json",
                    "dataType":"json"
                },
                success: function(resp) {
                    var realRateValue = parseFloat(resp.data.data.rate);
                    if (realRateValue){
                        self.setData({
