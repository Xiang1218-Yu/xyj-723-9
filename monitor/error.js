...
/** 手动上报一条错误 */
function reportError(name, error, extra) {
    const message = error instanceof Error ? error.message : String(error);
    const stack = error instanceof Error ? error.stack : undefined;
    (0, reporter_1.report)({ type: 'error', name, extra: { message, stack, ...extra } });
}
/**
...
function initErrorMonitor() {
    ...
    if (typeof wx.onError === 'function') {
        wx.onError((err) => {
            ...
            (0, reporter_1.report)({ type: 'error', name: 'jsError', extra: { message } });
        });
    }
    ...
    if (typeof wx.onAppHide === 'function') {
        wx.onAppHide(() => (0, reporter_1.flush)());
    }
}
